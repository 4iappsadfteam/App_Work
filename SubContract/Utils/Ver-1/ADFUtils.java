...
public class ADFUtils implements java.io.Serializable {
    ...
    */
    public static void refreshTable(String unique_name, ViewObject vo){
    ...
    public static String getFunctionDetails(String type,String col) { //FuncId //WfProcessUrl
        String ret = null;
        ViewObject funcVO = ADFUtils.findIterator("FunctionROVO1Iterator").getViewObject();
        funcVO.setNamedWhereClauseParam("p_code",type);
        funcVO.executeQuery();
        System.err.println("===f==" + funcVO.getEstimatedRowCount());
        if (funcVO.getEstimatedRowCount() > 0) {
            ret =
    funcVO.first().getAttribute(col) != null ? funcVO.first().getAttribute(col).toString() :
    "";
        }
        return ret;
    }
